...
contract('HashedTimelockERC20', accounts => {
  ...
  before(async () => {
    ...
    console.log("sender=",await token.balanceOf.call(sender))
    console.log("receiver=",await token.balanceOf.call(receiver))
...
*/
  ...
  const newContract = async ({
    ...
    console.log("balance before transfer/approve:")
    console.log("sender=",await token.balanceOf.call(sender))
    console.log("receiver=",await token.balanceOf.call(receiver))
    await token.approve(htlc.address, tokenAmount, {from: sender})
    await token.transfer(htlc.address, tokenAmount, {from: sender}) //Change to transfer
    console.log("balance after transfer/approve before withdraw:")
    console.log("sender=",await token.balanceOf.call(sender))
    console.log("receiver=",await token.balanceOf.call(receiver))
